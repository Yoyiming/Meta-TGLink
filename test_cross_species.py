...
parser.add_argument('--inner_lr', type=float, default=0.0001, help='Inner loop learning rate')
parser.add_argument('--seed', type=int, default=42, help='Random seed')
parser.add_argument('--sample', type=str, default='sample2', help='sample')
parser.add_argument('--train_cell', type=str, default='mESC', help='cell line name')
...
cell_lines = ['A549', 'A375', 'HEK293T', 'PC3'] + [args.train_cell]
...
for cell_line in cell_lines:
    if cell_line == cell_lines_train:
        train_data_path = f'cell_line_dataset/Specific/{cell_line} 1000/{args.sample}_train.csv'
        val_data_path = f'cell_line_dataset/Specific/{cell_line} 1000/{args.sample}_val.csv'
        test_data_path = f'cell_line_dataset/Specific/{cell_line} 1000/{args.sample}_test.csv'
    else:
    ...
    for data_path, data_loaders in zip([train_data_path, val_data_path, test_data_path],
        ...
        if data_path == test_data_path:
            if cell_line == cell_lines_train:
                dataset = MetaDataset(data, k_shot, data.shape[0] - k_shot, n_way)
                ...
                dataloader = DataLoader(dataset, batch_size=1, shuffle=False)
                data_loaders[cell_line] = dataloader
        elif data_path == train_data_path and cell_line == cell_lines_train:
            dataset = MetaDataset(data, k_shot, k_query, n_way)
            dataloader = DataLoader(dataset, batch_size=1, shuffle=False)
            data_loaders[cell_line] = dataloader
...
for cell_line in cell_lines:
    if cell_line == cell_lines_train:
        data_path = f'expression matrix/Specific Dataset/{cell_line}/TFs+1000/BL--ExpressionData.csv'
    else:
...
model.load_state_dict(torch.load(model_path + '/Meta_TGLink_hm' + '_' + cell_lines_train + '_' + args.sample + '_' + str(k) +'.pt'))
...
for cell_line in cell_lines:
    ...
    if cell_line == cell_lines_train:
        ...
        np.savetxt(f'{save_dir}/hm_zero-shot_{k}.txt', np.array([auc, aupr]), fmt='%.4f')
        ...
        for step, (spt_set, qry_set) in enumerate(test_loader):
            ...
            for task in range(task_num):
                ...
                # spt_data.to_csv(f'cell_line_dataset/oe/{cell_line}/{args.sample}_hm_spt.csv', index=False)
                # qry_data.to_csv(f'cell_line_dataset/oe/{cell_line}/{args.sample}_hm_qry.csv', index=False)
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        np.savetxt(f'{save_dir}/hm_few-shot_{cell_line}_{k}.txt', np.array([auc, aupr]), fmt='%.4f')
